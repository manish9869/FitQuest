...
import {
    ...
    ChevronRight, X, Sparkles, Layers, ChefHat, Dumbbell, Apple, BookOpen, Star, Database, LogOut,
    Sun, Moon
} from 'lucide-react';
...
export default function AdminLayout() {
    ...
    return (
        <div className="min-h-screen bg-background flex">
            ...
            <aside className={`fixed lg:sticky top-0 left-0 h-screen w-64 z-50 flex flex-col transform transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} bg-sidebar border-r border-sidebar-border`}>
                            ...
                            {group.items.map(item => {
                                ...
                                return (
                                    <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)}
                                        className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 group relative ${active ? 'bg-purple-500/15 text-purple-500' : 'text-muted-foreground hover:text-foreground hover:bg-sidebar-accent'}`}>
                ...
                <div className="p-3 border-t border-white/5 space-y-1">
                    ...
                    {/* <Link to="/dashboard" className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-sidebar-accent transition-all">
                        <ArrowLeft className="w-4 h-4" /> Back to Dashboard
                    </Link> */}
                    <button onClick={logout}
                        className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-red-500 hover:bg-red-500/10 transition-all w-full">
            ...
            <main className="flex-1 min-h-screen overflow-x-hidden">
                <header className="sticky top-0 z-30 px-4 lg:px-8 h-16 flex items-center justify-between bg-sidebar/95 backdrop-blur-xl border-b border-sidebar-border">
                    <div className="flex items-center gap-3">
                        <button className="lg:hidden p-2 rounded-lg hover:bg-sidebar-accent" onClick={() => setSidebarOpen(true)}>
