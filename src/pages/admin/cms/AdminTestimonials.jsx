...
import { Plus, Pencil, Trash2, Star, MessageSquare, Loader2, X, Upload, Eye, EyeOff, Search } from 'lucide-react';
...
function TestimonialDrawer({ open, onClose, item }) {
    ...
    const save = useMutation({
        mutationFn: (d) => {
            const clean = {
                ...d,
                weight_change_kg: Number(d.weight_change_kg) || undefined,
    ...
    const ImageUpload = ({ field, loading, setLoading, label }) => (
        <div>
            ...
            {form[field] ? (
                ...
                <label className="flex flex-col items-center justify-center w-full h-24 rounded-xl border-2 border-dashed border-white/10 hover:border-pink-500/40 cursor-pointer transition-colors">
...
export default function AdminTestimonials() {
    ...
    const filtered = items.filter(t => {
        ...
        const matchRating = filterRating === 'all' || (t.rating || 5) >= Number(filterRating);
        const matchStatus = filterStatus === 'all' ||
            (filterStatus === 'published' ? t.is_published : !t.is_published);
        const matchFeatured = filterFeatured === 'all' ||
            (filterFeatured === 'featured' ? t.is_featured : !t.is_featured);
        return matchSearch && matchRating && matchStatus && matchFeatured;
    });
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold font-space flex items-center gap-2">
                        <MessageSquare className="w-6 h-6 text-pink-400" /> Testimonials
                    </h1>
                    <p className="text-sm text-muted-foreground mt-1">{items.length} transformation stories · {filtered.length} shown</p>
                </div>
            ...
            {isLoading ? (
                ...
                <div className="text-center py-16 text-muted-foreground">
                    ...
                    <p>{search || filterStatus !== 'all' || filterFeatured !== 'all' || filterRating !== 'all'
                        ...
                        <div key={t.id} className="glass rounded-xl p-4 flex flex-col gap-3 group hover:bg-white/5 transition-all">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button size="icon" variant="ghost" onClick={() => { setEditing(t); setDrawerOpen(true); }} className="h-7 w-7 hover:bg-blue-500/10 hover:text-blue-400">
                                        <Pencil className="w-3 h-3" />
                                    </Button>
                                    <Button size="icon" variant="ghost" onClick={() => deleteMutation.mutate(t.id)} className="h-7 w-7 hover:bg-red-500/10 hover:text-red-400">
                            ...
                            <div className="flex">
                                {[1, 2, 3, 4, 5].map(n => (
                                    <Star key={n} className={`w-3 h-3 ${(t.rating || 5) >= n ? 'text-yellow-400 fill-yellow-400' : 'text-muted-foreground'}`} />
                                ))}
                            </div>
                            ...
                            <div className="flex items-center gap-3 mt-auto">
                                {t.weight_change_kg && (
                                    <span className="text-xs text-emerald-400 font-semibold">
                                        {t.weight_change_kg > 0 ? '+' : ''}{t.weight_change_kg}kg
                                    </span>
                                )}
                                {t.duration_weeks && <span className="text-xs text-muted-foreground">{t.duration_weeks}w</span>}
                                {t.is_featured && <Star className="w-3 h-3 text-yellow-400 fill-yellow-400" />}
                                <button
                                    onClick={() => togglePublish.mutate({ id: t.id, val: !t.is_published })}
                                    className={`ml-auto flex items-center gap-1 text-xs ${t.is_published ? 'text-emerald-400' : 'text-muted-foreground'}`}
                                >
