...
function ProgramDrawer({ open, onClose, program }) {
    ...
    React.useEffect(() => { setForm(program ? { ...EMPTY, ...program, included_features: program.included_features?.length ? program.included_features : EMPTY_FEATURES } : EMPTY); }, [program, open]);
    const set = (k, v) => setForm(p => ({ ...p, [k]: v }));

    const save = useMutation({
        mutationFn: (d) => {
            const clean = { ...d, duration_weeks: Number(d.duration_weeks) || undefined, price: Number(d.price) || undefined, original_price: Number(d.original_price) || undefined, included_features: d.included_features.filter(Boolean) };
    ...
    const handleImageUpload = async (e) => {
        ...
        set('banner_image_url', file_url); setUploading(false);
    };
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            ...
                            {form.banner_image_url ? (
                                <div className="relative w-full h-32 rounded-xl overflow-hidden">
                                    ...
                                    <button type="button" onClick={() => set('banner_image_url', '')} className="absolute top-2 right-2 bg-black/60 rounded-full p-1"><X className="w-3 h-3" /></button>
                                </div>
                            ) : (
                            ...
                            <Input value={form.target_audience} onChange={e => set('target_audience', e.target.value)} placeholder="Target Audience (e.g. Beginners, Women 30+)" className="bg-white/5 border-white/10" />
                            <div className="grid grid-cols-3 gap-3">
                                <div>
                                    <label className="text-xs text-muted-foreground mb-1 block">Category</label>
                                    <select value={form.category} onChange={e => set('category', e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-foreground">
                                        {['fat_loss', 'muscle_gain', 'transformation', 'home_workout', 'athletic', 'beginner'].map(c => <option key={c} value={c}>{c.replace('_', ' ')}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-xs text-muted-foreground mb-1 block">Difficulty</label>
                                    <select value={form.difficulty} onChange={e => set('difficulty', e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-foreground">
                            ...
                            <div>
                                ...
                                <select value={form.required_plan} onChange={e => set('required_plan', e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-foreground">
                                    {['free', 'basic', 'premium', 'elite'].map(p => <option key={p} value={p}>{p}</option>)}
                                </select>
                            </div>
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-wider">Included Features</h3>
                                    <Button type="button" size="sm" variant="ghost" onClick={addFeature} className="text-emerald-400 hover:bg-emerald-500/10"><Plus className="w-3 h-3" /></Button>
                                </div>
                                {form.included_features.map((f, i) => (
                                    <div key={i} className="flex gap-2">
                                        ...
                                        <Button size="icon" variant="ghost" onClick={() => removeFeature(i)} className="h-9 w-9 text-red-400 hover:bg-red-500/10 flex-shrink-0"><X className="w-3 h-3" /></Button>
                                    </div>
                                ))}
                            </div>
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold font-space flex items-center gap-2"><Target className="w-6 h-6 text-purple-400" /> Programs</h1>
                    <p className="text-sm text-muted-foreground mt-1">{programs.length} programs</p>
                </div>
                            ...
                            <div className="p-4">
                                <div className="flex items-start justify-between">
                                    ...
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <Button size="icon" variant="ghost" onClick={() => { setEditing(p); setDrawerOpen(true); }} className="h-7 w-7 hover:bg-blue-500/10 hover:text-blue-400"><Pencil className="w-3 h-3" /></Button>
                                        <Button size="icon" variant="ghost" onClick={() => deleteMutation.mutate(p.id)} className="h-7 w-7 hover:bg-red-500/10 hover:text-red-400"><Trash2 className="w-3 h-3" /></Button>
                                    </div>
                                </div>
                                <div className="flex flex-wrap gap-1.5 mt-3">
                                    ...
                                    <span className={`text-xs flex items-center gap-0.5 ${p.is_published ? 'text-emerald-400' : 'text-muted-foreground'}`}>{p.is_published ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}</span>
                                </div>
                            </div>
                        </div>
                    ))}
            ...
            {!isLoading && filtered.length === 0 && (
                <div className="text-center py-16 text-muted-foreground"><Target className="w-12 h-12 mx-auto mb-3 opacity-20" /><p>No programs yet.</p></div>
            )}
