...
function BlogDrawer({ open, onClose, post }) {
    ...
    const handleImageUpload = async (e) => {
        ...
        set('cover_image_url', file_url); setUploading(false);
    };
                        ...
                        <div className="flex-1 overflow-y-auto p-5 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <select value={form.category} onChange={e => set('category', e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-foreground">
                                        {['nutrition', 'fat_loss', 'recovery', 'supplements', 'training_tips', 'mindset'].map(c => <option key={c} value={c}>{c.replace('_', ' ')}</option>)}
                                    </select>
                                </div>
                                <div>
                            ...
                            <div>
                                ...
                                <div className="flex gap-2">
                                    <Input value={tagInput} onChange={e => setTagInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), addTag())} placeholder="Add tag..." className="bg-white/5 border-white/10" />
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea value={form.seo_description} onChange={e => set('seo_description', e.target.value)} placeholder="SEO Description (160 chars)" maxLength={160}
                                    className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground resize-none h-16" />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold font-space flex items-center gap-2"><FileText className="w-6 h-6 text-yellow-400" /> Blog & Content</h1>
            ...
            <div className="glass rounded-xl overflow-hidden">
                {isLoading ? <div className="p-12 text-center text-muted-foreground">Loading...</div> : filtered.length === 0 ? (
                    <div className="p-12 text-center"><FileText className="w-12 h-12 text-muted-foreground mx-auto mb-3 opacity-30" /><p className="text-muted-foreground">No articles yet.</p></div>
                ) : (
                            ...
                            <tbody>
                                {filtered.map((p, i) => (
                                    <tr key={p.id} className={`border-b border-white/5 hover:bg-white/3 transition-colors ${i % 2 === 0 ? '' : 'bg-white/2'}`}>
                                        ...
                                        <td className="px-4 py-3"><span className={`text-xs capitalize ${CAT_COLORS[p.category]}`}>{p.category?.replace('_', ' ')}</span></td>
                                        <td className="px-4 py-3 text-muted-foreground text-xs">{p.author_name || '—'}</td>
                                        <td className="px-4 py-3 text-muted-foreground text-xs">{p.published_at ? format(new Date(p.published_at), 'MMM d, yyyy') : '—'}</td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <Button size="icon" variant="ghost" onClick={() => { setEditing(p); setDrawerOpen(true); }} className="h-7 w-7 hover:bg-blue-500/10 hover:text-blue-400"><Pencil className="w-3.5 h-3.5" /></Button>
                                                <Button size="icon" variant="ghost" onClick={() => deleteMutation.mutate(p.id)} className="h-7 w-7 hover:bg-red-500/10 hover:text-red-400"><Trash2 className="w-3.5 h-3.5" /></Button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
