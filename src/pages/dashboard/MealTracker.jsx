...
export default function MealTracker() {
    ...
    const { data: meals = [] } = useQuery({ queryKey: ['meals', selectedDate, user?.email], queryFn: () => entities.MealLog.filter({ user_email: user?.email, date: selectedDate }), enabled: !!user?.email });
    const { data: allMeals = [] } = useQuery({ queryKey: ['meals-history', user?.email], queryFn: () => entities.MealLog.filter({ user_email: user?.email }), enabled: !!user?.email });

    const addMeal = useMutation({
        mutationFn: (data) => entities.MealLog.create({ ...data, user_email: user.email, date: selectedDate }),
        onSuccess: () => {
    ...
    const addMeals = useMutation({
        mutationFn: async (items) => {
            for (const item of items) {
                await entities.MealLog.create({ ...item, user_email: user.email, date: selectedDate });
            }
        },
        onSuccess: () => {
            qc.invalidateQueries({ queryKey: ['meals'] });
    ...
    const totalAllCals = useMemo(() => allMeals.reduce((s, m) => s + (m.calories || 0), 0), [allMeals]);
    const avgDailyCals = useMemo(() => {
        const dates = new Set(allMeals.map(m => m.date));
        return dates.size ? Math.round(totalAllCals / dates.size) : 0;
    }, [allMeals, totalAllCals]);
        ...
        <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
                <TrackerHistoryChart
                    ...
                    unit="kcal"
                    goal={calTarget}
                    goalLabel={`Target: ${calTarget}kcal`}
                    type="area"
                />
                <TrackerHistoryChart
                    logs={allMeals}
                    ...
                    color="#f97316"
                    unit="g"
                    goal={profile?.protein_target || 150}
                    goalLabel={`Target: ${profile?.protein_target || 150}g`}
                    type="bar"
                />
            </div>
