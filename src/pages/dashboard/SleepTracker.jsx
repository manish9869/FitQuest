...
export default function StepTracker() {
    ...
    const { data: profiles } = useQuery({ queryKey: ['userProfile', user?.email], queryFn: () => base44.entities.UserProfile.filter({ user_email: user?.email }), enabled: !!user?.email });
    const profile = profiles?.[0];

    const { data: logs = [] } = useQuery({ queryKey: ['steps', selectedDate, user?.email], queryFn: () => base44.entities.StepLog.filter({ user_email: user?.email, date: selectedDate }), enabled: !!user?.email });
    const { data: allLogs = [] } = useQuery({ queryKey: ['steps-history', user?.email], queryFn: () => base44.entities.StepLog.filter({ user_email: user?.email }), enabled: !!user?.email });

    const addSteps = useMutation({
        mutationFn: (steps) => base44.entities.StepLog.create({ user_email: user.email, date: selectedDate, steps, calories_burned: Math.round(steps * 0.04) }),
        onSuccess: () => { qc.invalidateQueries({ queryKey: ['steps'] }); setStepInput(''); },
    });

    const deleteLog = useMutation({
        mutationFn: (id) => base44.entities.StepLog.delete(id),
